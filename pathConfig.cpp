...
void pathConfig_c::read_f(const QJsonObject& json_par_con)
{
    ...
    dateTimeFormat_pri = json_par_con["dateTimeFormat"].toString();
...
void pathConfig_c::writeJSONDocumented_f(QJsonObject& json_par)
{
    ...
    json_par["monitorIntervalMilliseconds"] = "interval in milliseconds, to check for filesystem changes in the path, this value is a number but the field is a string because json number limitations";
    json_par["sizeIncreaseCycleThreshold"] = "how many cycles of monitorIntervalMilliseconds without a file size increasing to assume a file size stopped increasing, the reverse is used to assume it's increasing";
    json_par["sizeDecreaseCycleThreshold"] = "how many cycles of monitorIntervalMilliseconds without a file size decreasing to assume a file size stopped decreasing, the reverse is used to assume it's decreasing";
    ...
    changesToMonitorArrayTmp.append(QJsonValue("newFileFinishedLoading, this implies \"newFile\" and file size constantly growing between intervals, within \"monitorIntervalMilliseconds\" and \"newFileStoppedGrowingTimeoutMilliseconds\" because the first hard limits the second, when the growing stops send a notification"));
    ...
    json_par["passNotificationTextAsArgument"] = "pass the notification text as a single argument to the process";
    json_par["dateTimeFormat"] = "see QDatetime::toString documentation, yyyy-MM-dd hh:mm:ss.zzz by default";
    json_par["UTC"] = "true: show time in UTC, false: show in local, false by default";
    json_par["threadFileSystemScan"] = "true: make an extra thread when scanning the filesystem, one extra per each pathConfig object being monitored, false: use the main thread, false by default";
}
